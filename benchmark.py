...
def bench_algorithms(args: argparse.Namespace) -> int:
    ...
    for n in args.limits:
        ...
        if tag == "synthetic":
            ...
            source = f"synthetic:{args.synthetic_mode} unique={unique_for_n}"
            if args.synthetic_mode == "zipf":
                source += f" alpha={args.zipf_alpha}"
            elif args.synthetic_mode == "hotspot":
                source += f" hot_fraction={args.hot_fraction} hot_traffic_share={args.hot_traffic_share}"
            ...
            def iter_keys(seed: int):
                ...
                return generate_hotspot_keys(
                    n,
                    num_ips=unique_for_n,
                    hot_fraction=args.hot_fraction,
                    hot_traffic_share=args.hot_traffic_share,
                    seed=seed,
                )
    ...
    with open(meta_path, "w", encoding="utf-8") as f:
        ...
        f.write(f"hot_fraction={getattr(args, 'hot_fraction', None)}\n")
        f.write(f"hot_traffic_share={getattr(args, 'hot_traffic_share', None)}\n")
...
def parse_args() -> argparse.Namespace:
    ...
    pa.add_argument("--runs", type=int, default=10, help="Repeats per setting (proposal spec: 10)")
    ...
    pa.add_argument("--synthetic-mode", choices=["uniform", "zipf", "hotspot"], default="hotspot", help="Synthetic distribution")
    ...
    pa.add_argument("--hot-fraction", type=float, default=0.05, help="Fraction of IPs in the hot set (proposal: 0.05)")
    pa.add_argument("--hot-traffic-share", type=float, default=0.8, help="Traffic share generated by hot set (proposal: 0.8)")
